...
export async function generateMetadata({
  ...
  const post = await fetchPostBySlug(CATEGORY.caseStudies[locale as Locale], slug);
  if (!post) {
    const t = await getTranslations({ locale, namespace: "caseStudyPost" });
  ...
  const localizedUrl = (l: string, postSlug: string) =>
    `${SITE.url}${l === routing.defaultLocale ? "" : `/${l}`}/case-studies/${postSlug}`;
...
export default async function CaseStudyPostPage({
  ...
  // slugs, so guessing "/case-studies/<this-english-slug>" would 404.
  const altPath = altPost ? `/case-studies/${altPost.slug}` : "/case-studies";

  const title = decode(post.title.rendered);
  const canonicalUrl = `${SITE.url}${locale === routing.defaultLocale ? "" : `/${locale}`}/case-studies/${slug}`;
    ...
    <>
      ...
      <section className="container-px pt-24 pb-8">
        <Reveal className="max-w-3xl">
          <Link href="/case-studies" className="inline-flex items-center gap-1.5 text-sm text-fg-muted hover:text-fg transition-colors">
            <ArrowLeft size={15} />
            {t("backToCaseStudies")}
          </Link>
